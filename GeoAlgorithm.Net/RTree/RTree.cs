...
namespace GeoAlgorithm.Net.RTree
{
    ...
    public class RTree<T>
    {
        ...
        private void Insert(RTreeNode<T> item, int level)
        {
            ...
            node.Envelope.ExpandToInclude(envelope);
        ...
        private RTreeNode<T> ChooseSubtree(Envelope bbox, RTreeNode<T> node, int level, List<RTreeNode<T>> path)
        {
            while (true)
            {
                ...
                {
                    var child = node.Children[i];
                    var area = (int)child.Envelope.Area;
        ...
        private int ChooseSplitIndex(RTreeNode<T> node, int minEntries, int totalCount)
        {
            ...
            for (var i = minEntries; i <= totalCount - minEntries; i++)
            {
                ...
                var area = (int)(bbox1.Area + bbox2.Area);
        ...
        public void Remove(T item, Envelope envelope)
        {
            ...
            while (node != null || path.Count > 0)
            {
                ...
                {
                    // check current node
                    var it = node.Children.FirstOrDefault(n => Comparer.Equals(item, n.Data));
        ...
        private static Envelope SumChildBounds(RTreeNode<T> node, int startIndex, int endIndex)
        {
            ...
            {
                retval.ExpandToInclude(node.Children[i].Envelope);
            }
        ...
        private static void AdjutsParentBounds(Envelope bbox, List<RTreeNode<T>> path, int level)
        {
            ...
            {
                path[i].Envelope.ExpandToInclude(bbox);
            }
        }
        ...
        private static int CompareNodesByMinX(RTreeNode<T> a, RTreeNode<T> b) { return a.Envelope.MinX.CompareTo(b.Envelope.MinX); }
        private static int CompareNodesByMinY(RTreeNode<T> a, RTreeNode<T> b) { return a.Envelope.MinY.CompareTo(b.Envelope.MinY); }
        ...
        private static int AllDistMargin(RTreeNode<T> node, int m, int M, Comparison<RTreeNode<T>> compare)
        {
            ...
            {
                var child = node.Children[i];
                leftBBox.ExpandToInclude(child.Envelope);
            ...
            {
                var child = node.Children[i];
                rightBBox.ExpandToInclude(child.Envelope);
