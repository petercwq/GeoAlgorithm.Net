...
namespace Trimble.Ems.Cross.Types
{
    ...
    /// </summary>
    public class LRUCache<TKey, TValue> : IEnumerable<KeyValuePair<TKey, LinkedListNode<TValue>>> where TValue : class, IDisposable
    {
        ...
        public LRUCache(int entryLimit, int sizeLimit, Func<TValue, int> slotSizer)
        {
            if (sizeLimit != 0 && slotSizer == null)
                throw new ArgumentException("If sizeLimit is set, the slotSizer must be provided", "slotSizer");
        ...
        public int Capacity
        {
            get { return entryLimit; }
            set { entryLimit = value; }
        }
        ...
        void Evict()
        {
           ...
           Debug.WriteLine("Evicted, got: {0} bytes and {1} slots", currentSize, list.Count);
        }
        ...
        public TValue this[TKey key]
        {
            ...
            set
            {
                ...
                revdict[node] = key;
                currentSize += size;
                Debug.WriteLine("new size: {0} with {1}", currentSize, list.Count);
            }
        }
        ...
        #region IEnumerable<KeyValuePair<TKey,LinkedListNode<TValue>>> Members

        public IEnumerator<KeyValuePair<TKey, LinkedListNode<TValue>>> GetEnumerator()
        {
